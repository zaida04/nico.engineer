        ...
        <>
            ...
            <PageTransition>
                <div className="text-white mb-2">
                    ...
                    <p>
                        Clear, concise explanations of popular AWS services. This was created because I noticed a lot of people at school had difficulty
                        understanding why AWS has so many services and how similar ones are actually different from each other. Because of the
                        beginner-intermediate target audience, I&apos;ve tried to keep the explanations as simple as possible. If you have any suggestions or
                        feedback, reach out to me through GitHub.
                    </p>
                </div>
                ...
                <div className="mb-6">
                    <input
                        ...
                        className="w-full p-4 pl-8 text-md rounded-lg bg-slate-800 text-gray-100"
                        placeholder="Search for a specific service..."
                    />
                    <a className="text-white mt-1 text-sm hover:underline" href="https://github.com/zaida04/nico.engineer/issues/new">
                        Missing a service we should have? Let me know.
                    </a>
                </div>
...
export function ServiceResults(props: { searchQuery: string; searchResults: typeof AWS_SERVICES[number][] }) {
            ...
            <>
                {everyThree.map((three, i) => (
                    <div key={i} className="grid gap-4">
                        {three.map((service, i) => (
                            <ServicePreview key={i} name={service.name} tags={service.tags} description={service.description} />
                        ))}
                    </div>
                ))}
            </>
    ...
    return (
        <>
            {props.searchResults.map((service, i) => (
                <ServicePreview key={i} name={service.name} tags={service.tags} description={service.description} />
            ))}
        </>
    );
